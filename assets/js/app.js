...
function normalizeQuestion(q) {
    ...
    return {
        ...
        // Preserve curriculum metadata used by the 4-level Explore vocabulary flow.
        // rawItemToFlatQuestion() already carries these fields; normalizeQuestion() must not drop them.
        curriculum_level: Number(q.curriculum_level ?? 0),
        unit_anchor: Number(q.unit_anchor ?? 0),
        unit_group: q.unit_group ?? '',
        curriculum_role: q.curriculum_role ?? '',
        semantic_group: q.semantic_group ?? ''
    };
}
...
function rawItemToFlatQuestion(sk, it, allWordsPool, sectionLabel) {
    ...
    const base = { id: it.question_id, sub: label, sub_code: sk, w: weekCode, tag: skill, img: it.image_url || '', emo: it.emoji || '', aud: it.audio_text || '', pg: it._paired_group || '', oipa: it.options_ipa || null, curriculum_level: Number(it.curriculum_level || 0), unit_anchor: Number(it.unit_anchor || 0), unit_group: it.unit_group || '', curriculum_role: it.curriculum_role || '', semantic_group: it.semantic_group || '' };
        ...
        section.topics.forEach(topic => {
            ...
            topic.questions = source.map((v, idx) => {
                ...
                const common = {
                    ...
                    curriculum_level: Number(v.curriculum_level || 0),
                    unit_anchor: Number(v.unit_anchor || 0),
                    unit_group: v.unit_group || '',
                    curriculum_role: v.curriculum_role || '',
                    semantic_group: v.semantic_group || '',
                    tts_locale: 'en-US'
                };
...
async function renderDashboardGrid() {
    ...
    TOPICS_CONFIG.forEach(t => {
        ...
        if ([2,3,4,5].includes(Number(t.id))) {
...
// ==========================================
// VOCABULARY HUB 4 LEVEL THEO 16 UNIT -> 3 CHẾ ĐỘ HỌC
// ==========================================
...
function getVocabularyCurriculumLevel_(q) {
    const explicit = Number(q?.curriculum_level || 0);
    if (explicit >= 1 && explicit <= 4) return explicit;
    const unit = Number(q?.unit_anchor || 0);
    return unit >= 1 && unit <= 16 ? Math.ceil(unit / 4) : 0;
}

function getVocabularySemanticGroup_(q) {
    const explicit = String(q?.semantic_group || '').trim();
    if (explicit) return explicit;
    const legacy = getVocabulary18TopicIds_(q)[0] || 0;
    return legacy ? `legacy_${legacy}` : '';
}

function getVocabularyLevelGroup_(level) {
    return EXPLORE_LEVEL_GROUPS.find(g => g.level === Number(level)) || null;
}
...
function renderVocabulary18Topics_() {
    // Tên hàm legacy được giữ để không phá các nơi đang gọi. UI thực tế là 4 level theo 16 Unit.
    activeVocabularyHubTopic = null;
    activeVocabularyStudyMode = 'flashcard';
    const content = document.getElementById('lecture-content');
    const contentWrap = content?.parentElement;
    ...
    flashcards.forEach(q => {
        const lv = getVocabularyCurriculumLevel_(q);
    ...
    list.innerHTML = `
        <div class="mb-3 text-center">
            <div class="text-base md:text-lg font-black text-violet-700">Vocabulary by 16 Units</div>
            <div class="text-xs md:text-sm font-bold text-slate-500 mt-1">4 levels · 40 words each · from the beginning to the end of Grade 2</div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 max-w-4xl mx-auto">
            ${EXPLORE_LEVEL_GROUPS.map((g,i) => {
                const style = SUBTOPIC_PALETTES[i % SUBTOPIC_PALETTES.length];
                return `<button onclick="openVocabularyLevel_(${g.level})" class="p-4 ${style.card} border-2 rounded-2xl font-bold text-left transition-all shadow-sm pastel-btn min-h-[88px]">
...
function openVocabularyLevel_(level) {
    ...
    if (!g) return;
    const pool = activeVocabularyHubQuestions.filter(q => q.sub_topic === '2.1' && getVocabularyCurriculumLevel_(q) === g.level);
    if (!pool.length) {
        showFriendlyAlert?.(`Level ${g.level} đang được cập nhật từ vựng.`, '📚');
        return;
    }
    activeVocabularyHubTopic = { ...g, id: g.level, title: `Level ${g.level} · ${g.name}`, vi: g.name_vi };
    activeVocabularyStudyMode = 'flashcard';
    vocabularyModeState = {
        flashcard: { answers: {}, wrongs: {} },
        picture: { answers: {}, wrongs: {} },
    ...
    updateNavTabs('2. Vocabulary', '📚', label, null, () => openVocabularyLevel_(g.level));
...
function renderVocabularySharedStudy_(q) {
    ...
    if (activeVocabularyStudyMode === 'flashcard') {
        ...
        body = `<div class="grid grid-cols-1 md:grid-cols-2 gap-3 items-stretch">
            <section class="rounded-3xl border-2 border-pink-200 bg-pink-50/40 p-2.5 shadow-sm flex flex-col">
                <div class="w-full aspect-[4/3] bg-white rounded-3xl overflow-hidden flex items-center justify-center mb-1" style="max-height:188px">${imageOrEmoji}</div>
                <div class="relative w-full flex flex-col items-center justify-center px-12 text-center" style="min-height:68px">
            ...
            <section class="rounded-3xl border-2 border-fuchsia-200 bg-gradient-to-br from-pink-50/60 via-white to-violet-50/60 p-2.5 shadow-sm">
                <div class="font-black text-fuchsia-600 text-sm md:text-base mb-1.5">⭐ 3 EXAMPLE SENTENCES:</div>
                <div class="space-y-1">${examples.map((ex,i)=>`<div class="bg-white rounded-2xl border border-fuchsia-100 px-3 py-1.5"><div class="flex items-center gap-2"><div class="flex-1 text-left font-bold text-slate-700">${escapeHtml(ex)}</div><button onclick="speakFlashcardExample(${i})" class="w-9 h-9 rounded-xl bg-sky-100 text-sky-600 shrink-0"><i class="fa-solid fa-volume-high text-xs"></i></button></div><div class="mt-0.5 text-left text-xs md:text-sm font-semibold text-slate-400">${escapeHtml(getVocabularyExampleVi_(q, ex))}</div></div>`).join('')}</div>
            </section>
        </div>`;
        ...
        q.answer = q.word;
        q.options = options;
        body = `<div class="min-h-[255px] flex flex-col items-center justify-center">
            <div class="text-xl md:text-2xl font-black text-slate-900 mb-1">Listen and choose.</div>
            <div class="text-xs md:text-sm font-bold text-slate-400 mb-2">Nghe và chọn từ đúng</div>
            <button onclick="speakVocabularyTarget_()" class="w-16 h-16 md:w-20 md:h-20 rounded-full bg-gradient-to-br from-pink-500 to-violet-500 text-white shadow-lg hover:scale-105 transition-transform flex items-center justify-center mb-3"><i class="fa-solid fa-volume-high text-3xl"></i></button>
...
function renderVocabularyModeTabs_() {
    if (activeVocabularyHubTopic?.level) openVocabularyLevel_(activeVocabularyHubTopic.level);
    else if (activeVocabularyHubTopic?.id) openVocabularyLevel_(activeVocabularyHubTopic.id);
}
...
// Cùng 4 level theo 16 Unit, không duy trì kho từ riêng.
// ==========================================
...
function renderSharedVocab18Topics_(activityId, pool) {
    // Tên hàm legacy được giữ; mục 3-4 nay dùng cùng 4 level của Vocabulary Master.
    const cfg = SHARED_VOCAB_ACTIVITY_CONFIG[activityId];
    const content = document.getElementById('lecture-content');
    const contentWrap = content?.parentElement;
    const speakBtn = document.querySelector('#view-lecture button[onclick="speakLecture()"]');
    ...
    list.innerHTML = `
        <div class="mb-3 text-center">
            <div class="text-base md:text-lg font-black text-violet-700">${escapeHtml(cfg.title.replace(/^\d+\.\s*/,''))} by 16 Units</div>
            <div class="text-xs md:text-sm font-bold text-slate-500 mt-1">Use the same Vocabulary Master · 4 progressive levels</div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 max-w-4xl mx-auto">
            ${EXPLORE_LEVEL_GROUPS.map((g,i) => {
                const style = SUBTOPIC_PALETTES[i % SUBTOPIC_PALETTES.length];
                return `<button onclick="openSharedVocabLevel_(${activityId},${g.level})" class="p-4 ${style.card} border-2 rounded-2xl font-bold text-left transition-all shadow-sm pastel-btn min-h-[88px]">
                    <div class="flex items-center justify-between gap-3">
                        <div><div class="text-lg md:text-xl font-black ${style.num}">${g.icon} Level ${g.level} · ${g.name}</div><div class="text-xs md:text-sm text-slate-500 font-extrabold mt-1">${g.name_vi} · Unit ${g.units[0]}–${g.units[g.units.length-1]}</div></div>
                        <span class="text-xs font-black ${style.badge} px-3 py-1 rounded-full border shrink-0">${counts[g.level] || 0} words</span>
                    </div>
                </button>`;
            }).join('')}
...
async function openSharedVocabLevel_(activityId, level) {
    const cfg = SHARED_VOCAB_ACTIVITY_CONFIG[activityId];
    const g = getVocabularyLevelGroup_(level);
    if (!cfg || !g) return;
    const all = await getSharedVocabularyBasePool_();
    const pool = all.filter(q => getVocabularyCurriculumLevel_(q) === g.level);
    if (!pool.length) return alert(`Level ${g.level} chưa có từ vựng.`);
    activeSharedVocabActivity = { activityId, topic: { ...g, id: g.level, level: g.level } };
    launchSharedVocabActivity_(activityId, pool, `Level ${g.level} · ${g.name} / ${g.name_vi} · Unit ${g.units[0]}–${g.units[g.units.length-1]}`);
}
...
function launchSharedVocabActivity_(activityId, vocabPool, label) {
    ...
    updateNavTabs(cfg.title, cfg.icon, label, null, () => {
        ...
        if (t?.level) openSharedVocabLevel_(activityId, t.level);
        else openSharedVocabActivityHub_(activityId);
    });
...
function makeOddOneOutQuestion_(v, vocabPool, idx) {
    const mainGroup = getVocabularySemanticGroup_(v);
    if (!mainGroup) return null;
    const same = shuffleArray(vocabPool.filter(x => x.word && x.word !== v.word && getVocabularySemanticGroup_(x) === mainGroup)).slice(0,3);
    const other = shuffleArray(vocabPool.filter(x => x.word && getVocabularySemanticGroup_(x) && getVocabularySemanticGroup_(x) !== mainGroup))[0];
...
function splitOddOneOutLevels_(pool) {
    const buckets = [[], [], [], []];
    pool.forEach(v => {
        const level = getVocabularyCurriculumLevel_(v);
        if (level >= 1 && level <= 4) buckets[level - 1].push(v);
    });
    return buckets;
}
...
function handleTa2PracticeChoice_(btn,isCorrect,correctText){
    if(!btn||btn.disabled)return;
    const card=btn.closest('[data-ta2-practice-card]');
    const feedback=card?.querySelector('[data-ta2-practice-feedback]');
    if(isCorrect){
        card?.querySelectorAll('button[data-ta2-choice]').forEach(b=>{b.disabled=true;b.classList.add('opacity-75');});
        btn.classList.remove('border-pink-200','bg-white','opacity-75');
        btn.classList.add('bg-emerald-100','border-emerald-400','text-emerald-800');
        if(feedback){feedback.className='mt-2 rounded-xl bg-emerald-50 border border-emerald-200 px-3 py-2 text-xs font-extrabold text-emerald-700';feedback.textContent='✅ Great! Đúng rồi. Con đọc lại đáp án một lần nhé.';}
        if(correctText)speakEnglish(correctText,0.9);
    }else{
        btn.disabled=true;
        btn.classList.remove('border-pink-200','bg-white');
        btn.classList.add('bg-rose-100','border-rose-400','text-rose-700','opacity-70');
        if(feedback){feedback.className='mt-2 rounded-xl bg-amber-50 border border-amber-200 px-3 py-2 text-xs font-extrabold text-amber-700';feedback.textContent='💡 Chưa đúng. Con thử lại một đáp án khác nhé.';}
    }
}
function renderTa2LessonPage_(p){
    const ph=p.phonics||{}, voc=p.vocabulary||[], dialog=p.mini_dialogue||[];
    const words=(ph.words||[]).map(w=>`<button onclick="speakEnglish('${escapeJsString_(w)}',0.88)" class="lesson-chip hover:border-violet-400">${escapeHtml(w)} <span class="ml-1 text-[10px]">🔊</span></button>`).join('');
    const vocab=voc.map(v=>`<div class="rounded-xl bg-white border border-pink-100 px-3 py-2 font-black text-slate-700"><div class="flex items-start justify-between gap-2"><div>${escapeHtml(v.word||'')}<div class="text-[10px] font-bold text-slate-400 mt-0.5">${v.ipa?`/${escapeHtml(String(v.ipa).replace(/^\/+|\/+$/g,''))}/ · `:''}${escapeHtml(v.meaning||'')}</div></div><button onclick="speakEnglish('${escapeJsString_(v.word||'')}',0.88)" class="shrink-0 w-8 h-8 rounded-lg bg-pink-50 text-pink-600 border border-pink-100" title="Nghe từ">🔊</button></div></div>`).join('');
    const dlgVi=p.mini_dialogue_vi||[]; const dlg=dialog.map((x,i)=>`<div class="rounded-xl bg-purple-50 border border-purple-100 px-3 py-2 text-sm font-bold text-slate-700"><div class="flex items-start justify-between gap-2"><div>${escapeHtml(x)}${dlgVi[i]?`<div class="text-[10px] text-slate-400 mt-1">${escapeHtml(dlgVi[i])}</div>`:''}</div><button onclick="speakEnglish('${escapeJsString_(x)}',0.9)" class="shrink-0 w-8 h-8 rounded-lg bg-white text-purple-600 border border-purple-100" title="Nghe câu">🔊</button></div></div>`).join('');
    return `<div class="space-y-3"><section class="rounded-3xl border-2 border-pink-200 bg-gradient-to-br from-pink-50 via-white to-purple-50 p-4"><div class="flex flex-col md:flex-row gap-4"><div class="md:w-[42%]"><img src="${escapeHtml(p.image||'')}" onerror="this.style.display='none'" class="w-full aspect-square object-cover rounded-2xl border border-pink-100 shadow-sm"><div class="mt-2 rounded-xl bg-white/80 border border-pink-100 px-3 py-2 text-[11px] font-extrabold text-slate-500">👀 Nhìn tranh → 🔊 nghe mẫu → 🗣️ nói theo</div></div><div class="md:w-[58%] space-y-3"><div class="flex items-start justify-between gap-2"><div><p class="text-sm md:text-base font-bold text-slate-600 leading-6">${escapeHtml(p.intro||'')}</p>${p.intro_vi?`<p class="text-[10px] font-bold text-slate-400 mt-1">${escapeHtml(p.intro_vi)}</p>`:''}</div><button onclick="speakBaiHocTa2_()" class="shrink-0 px-3 py-2 rounded-xl bg-pink-500 text-white text-xs font-black">🔊 Listen<span class="block text-[9px] opacity-80">Nghe toàn bài</span></button></div><div class="rounded-2xl bg-white border border-violet-100 p-3"><div class="text-xs font-black text-violet-600 mb-2">1 · 🔤 PHONICS <span class="text-slate-400">Nghe âm</span></div><div class="text-lg font-black text-violet-800">${escapeHtml(ph.letter||'')} · ${escapeHtml(ph.sound||'')}</div><div class="flex flex-wrap gap-2 mt-2">${words}</div></div><div class="rounded-2xl bg-white border border-fuchsia-100 p-3"><div class="text-xs font-black text-fuchsia-600 mb-2">2 · 📚 VOCABULARY <span class="text-slate-400">Nghe từ</span></div><div class="grid grid-cols-1 sm:grid-cols-3 gap-2">${vocab}</div></div></div></div></section><section class="rounded-2xl bg-gradient-to-br from-pink-50 to-violet-50 border border-fuchsia-200 p-4"><div class="flex items-start justify-between gap-3"><div><div class="text-xs font-black text-fuchsia-700 mb-1">3 · 💬 SENTENCE PATTERN<br><span class="text-[10px] text-slate-400">Mẫu câu</span></div><div class="text-lg md:text-xl font-black text-slate-800">${escapeHtml(p.sentence_pattern||'')}</div></div><button onclick="speakEnglish('${escapeJsString_(p.sentence_pattern||'')}',0.88)" class="shrink-0 px-3 py-2 rounded-xl bg-white text-fuchsia-700 border border-fuchsia-200 text-xs font-black">🔊 Nghe</button></div></section><section class="rounded-2xl bg-white border border-purple-100 p-4"><div class="font-black text-purple-700 mb-2">4 · 🐰 SPEAK WITH BUNNY<br><span class="text-[10px] text-slate-400">Nghe từng câu rồi nói theo</span></div><div class="space-y-2">${dlg}</div></section></div>`;
}
function renderTa2QuestionsPage_(p){
    const items=(p.items||[]).map((it,i)=>{
        if(it.type==='choice'){
            const correct=Number(it.answer);
            const correctText=(it.options||[])[correct]||'';
            const opts=(it.options||[]).map((o,j)=>`<button data-ta2-choice onclick="handleTa2PracticeChoice_(this,${j===correct},'${escapeJsString_(correctText)}')" class="text-left rounded-xl border border-pink-200 bg-white px-3 py-2.5 font-bold text-sm hover:border-fuchsia-300">${String.fromCharCode(65+j)}. ${escapeHtml(o)}</button>`).join('');
            return `<div data-ta2-practice-card class="rounded-2xl bg-pink-50/50 border border-pink-100 p-4"><div class="flex items-start gap-2 mb-2"><span class="w-6 h-6 shrink-0 rounded-full bg-pink-500 text-white flex items-center justify-center text-[11px] font-black">${i+1}</span><div class="font-black text-slate-800">${escapeHtml(it.prompt||'')}${it.prompt_vi?`<div class="text-[10px] text-slate-400 mt-1">${escapeHtml(it.prompt_vi)}</div>`:''}</div></div><div class="grid gap-2">${opts}</div><div data-ta2-practice-feedback class="hidden"></div></div>`;
        }
        const speakText=it.speak_text||it.tts_text||'';
        return `<div class="rounded-2xl bg-purple-50/60 border border-purple-100 p-4"><div class="flex items-start gap-2"><span class="w-6 h-6 shrink-0 rounded-full bg-purple-500 text-white flex items-center justify-center text-[11px] font-black">${i+1}</span><div class="font-black text-purple-700">🎙️ ${escapeHtml(it.prompt||'')}${it.prompt_vi?`<div class="text-[10px] text-slate-400 mt-1">${escapeHtml(it.prompt_vi)}</div>`:''}</div></div><div class="mt-3 rounded-xl bg-white border border-purple-100 px-3 py-2 text-sm font-extrabold text-slate-700">${escapeHtml(speakText)}</div><div class="mt-2 flex flex-wrap gap-2"><button onclick="speakEnglish('${escapeJsString_(speakText)}',0.88)" class="px-3 py-1.5 rounded-lg bg-purple-100 text-purple-700 text-xs font-black">🔊 Hear model<span class="block text-[9px]">Nghe mẫu</span></button><span class="px-3 py-1.5 rounded-lg bg-white border border-purple-100 text-[10px] font-extrabold text-slate-500">Nghe 1 lần → nói 2 lần</span></div></div>`;
    }).join('');
    return `<div class="space-y-3"><div class="rounded-2xl bg-sky-50 border border-sky-100 p-3 text-sm font-bold text-slate-600"><div class="font-black text-sky-700 mb-1">🎯 Practice flow · Luyện theo 3 bước</div>${escapeHtml(p.recall||'')}${p.recall_vi?`<div class="text-[10px] text-slate-400 mt-1">${escapeHtml(p.recall_vi)}</div>`:''}<div class="mt-2 text-[11px] font-extrabold text-slate-500">1. Nhận biết → 2. Chọn đúng → 3. Nghe và nói lại</div></div>${items}</div>`;
}
function renderTa2SummaryPage_(p,l){
    const pts=(p.key_points||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join('');
    const lines=(p.practice_lines||[]).map(x=>`<div class="rounded-xl bg-white border border-pink-100 px-3 py-2 font-black text-slate-700 flex items-center justify-between gap-2"><span>${escapeHtml(x)}</span><button onclick="speakEnglish('${escapeJsString_(x)}',0.88)" class="shrink-0 w-8 h-8 rounded-lg bg-pink-50 text-pink-600 border border-pink-100">🔊</button></div>`).join('');
    const challenge=p.transfer_challenge||p.finish_prompt||'';
    const challengeVi=p.transfer_challenge_vi||p.finish_prompt_vi||'';
    return `<div class="space-y-3"><section class="rounded-3xl bg-gradient-to-br from-pink-50 via-fuchsia-50 to-violet-50 border-2 border-fuchsia-200 p-4"><div class="text-xs font-black text-fuchsia-600">1 · 🧠 RECALL · NHỚ LẠI</div><div class="text-[11px] font-extrabold text-slate-400 mt-1">Thử nhớ trước khi nhìn đáp án.</div><ul class="list-disc pl-5 mt-3 space-y-2 text-sm md:text-base text-slate-700 font-semibold leading-7">${pts}</ul></section><section class="rounded-2xl bg-white border border-pink-100 p-4"><div class="font-black text-pink-700 mb-2">2 · 🗣️ USE · CON NÓI LẠI</div><div class="grid gap-2">${lines}</div></section><section class="rounded-2xl bg-violet-50 border border-violet-200 p-4"><div class="text-xs font-black text-violet-600 mb-1">3 · ⭐ MINI CHALLENGE</div><div class="font-black text-violet-800">${escapeHtml(challenge)}</div>${challengeVi?`<div class="text-[11px] text-slate-500 font-extrabold mt-1">${escapeHtml(challengeVi)}</div>`:''}</section><button onclick="markBaiHocTa2Complete_(${l.bai})" class="w-full py-3 rounded-2xl bg-gradient-to-r from-pink-500 to-purple-500 text-white font-black shadow-md">✅ Hoàn thành Bài ${l.bai}</button></div>`;
}
